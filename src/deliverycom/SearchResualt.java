...
public class SearchResualt extends javax.swing.JInternalFrame {
    ...
    public static Connection getConnection() throws SQLException {
     ...
     public List<PackageBean> SearchDB(String[] search) {
    List<PackageBean> result = new ArrayList<>();
    String query = "SELECT * FROM wsl_data WHERE wslNum LIKE ? AND deliveryGuyName LIKE ? AND costomerPhoneNum LIKE ?";
    try (PreparedStatement pstmt = con.prepareStatement(query)) {
        pstmt.setString(1, "%" + search[0] + "%");
        pstmt.setString(2, "%" + search[1] + "%");
        pstmt.setString(3, "%" + search[2] + "%");
    ...
    return result;
     ...
     public void handleSearch(String[] array) {
    List<PackageBean> searchResults = SearchDB(array);

    if (!searchResults.isEmpty()) {
        // Populate your JTable with searchResults
